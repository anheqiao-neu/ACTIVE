...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes):
        ...
        self.down_sample1 = conv22d(128, 256, 3, 2)  # self.backbone2 = darknet53()
        self.down_sample2 = conv22d(256, 512, 3, 2)
        self.make_five_conv1 = make_last_layers([256, 512], 512, len(anchors_mask[2]) * (num_classes + 5))
        self.make_five_conv2 = make_last_layers([512, 1024], 1024, len(anchors_mask[2]) * (num_classes + 5))

        self.c3_0=C3f(c1=1536,c2=512,n=1)
        self.c3_1 = C3f(c1=1024, c2=256, n=1)
        self.c3_2 = C3f(c1=512, c2=128, n=1)
        self.c3_3 = C3f(c1=512, c2=256, n=1)
        self.c3_4 = C3f(c1=1024, c2=512, n=1)
        # self.Conv_3 = nn.Conv2d(in_channels=512, out_channels=len(anchors_mask[2]) * (num_classes + 5), kernel_size=1, stride=1)
    def forward(self, x):
        #---------------------------------------------------#   
        #   获得三个有效特征层，他们的shape分别是：
        ...
        out1pan_branch = self.c3_3(out1pa_branch)
        out1 = self.make_five_conv1[5:](out1pan_branch)

        out0p_branch = self.down_sample2(out1pan_branch)
        out0pa_branch = torch.cat([out0p_branch, out0_branch], 1)
        out0pan_branch = self.c3_4(out0pa_branch)
        out0 = self.make_five_conv2[5:](out0pan_branch)
