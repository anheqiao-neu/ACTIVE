...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi=0, load_weights = False):
        ...
        self.last_layercat1 = make_last_layers([344, 688], 256+ out_filters2[-2] + 344,
        ...
        self.last_layer21 = make_last_layers([256, 512], out_filters2[-2] + 256,
                                             len(anchors_mask[1]) * (num_classes + 5))

        self.last_layer22_conv = conv2d(256, 128, 1)
        self.last_layer22_upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.last_layer22 = make_last_layers([128, 256], out_filters2[-3] + 128,
                                             len(anchors_mask[2]) * (num_classes + 5))

        # efficinet使用FPN
        self.last_layere0 = make_last_layers([512, 1024], 1024,len(anchors_mask[0]) * (num_classes + 5))

        self.last_layere1_conv = conv2d(512, 256, 1)
    ...
    def forward(self, x):
        ...
        out0_branch = self.c3_(out0_branch)
        ...
        out0 = self.last_layercat0[5:](out0pan_branch)
